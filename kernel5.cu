...
__device__ void njLoop(int flag, int *stateSpaces, int node, int ssIdx,
  ...
  for (int j = 0; j < noParentsInstances; j++) {
    ...
    for (int p2 = 1; p2 < noParents; p2++) {
      ...
      pid = parentArray[p2];
      // printf("pid + pidOffset = %d stateSpaceSize = %d", pid + pidOffset,
      // stateSpaceSize); printf("its working!!!!!!!!!!!\n");
      assert(pid + pidOffset < ppnLen);
      stateMatrixIdx[p2] =
          (j / div) % stateSpaces[pid + pidOffset]; //+pidOffset
...
__device__ double bDeu3(int flag, int blkoffset, int idxoffset, int tdxOffset,
                        ...
                        int *nij, int *nijk, int nijOs, int nijkOs,
                        int ppnLen) {
  ...
  if (blockIdx.x == uniSum - 1 || blockIdx.x == uniSum * 2 - 1) {
    ...
    noEdges1 =
        edgesPN[blockIdx.x + 1 - blkoffset] - edgesPN[blockIdx.x - blkoffset];
  }
...
//  stateVal = (int *)malloc(sizeof(int) * stateSpace[tdxOffset + threadIdx.x]); // ask 
  ...
  //	nijk = (int *)malloc(sizeof(int)*noParentInstances*stateSpace[tdxOffset
  //+ threadIdx.x]);

  // test = threadIdx.x+blockDim.x*blockIdx.x;

  //	 flag, int *stateSpaces, int node, int ssIdx, int blkId, int noGenes,int
  //samples, int noParents, int noParentsInstances, int *parentArray,int
  //*stateFlags, int *data,int *stateVal, int *stateMatrixIdx, int
  //*stateMatrixVal, int *out, int *out2)

  // test = njLoop(flag, stateSpace, threadIdx.x, tdxOffset + threadIdx.x,
  // noGenes, samples, noParents, noParentInstances, parentAry, stateFlag, data,
  // stateVal, nij, nijk, nijOs, nijkOs);
  njLoop(flag, stateSpace, threadIdx.x, tdxOffset + threadIdx.x, noGenes,
  ...
  for (int j = 0; j < noParentInstances; j++) {
    ...
    for (int k = 0; k < stateSpace[tdxOffset + threadIdx.x]; k++) {
      sum1 +=
          lgamma(
              double(
                  nijk[j * stateSpace[threadIdx.x + tdxOffset] + k + nijkOs]) +
              double(nPrime) / double(noParentInstances *
                                      stateSpace[threadIdx.x + tdxOffset])) -
          lgamma(double(nPrime) / double(noParentInstances *
                                         stateSpace[threadIdx.x + tdxOffset]));
    }
    sum2 += lgamma(double(nPrime) / double(noParentInstances)) -
            lgamma(double(nij[j + nijOs]) +
                   double(nPrime) / double(noParentInstances)) +
            sum1;
  }
  ...
  //	int sum2 = 0;
  //	for (int i = 0; i < stateSpace[threadIdx.x + tdxOffset] *
  //noParentInstances; i++){ 			sum2 += nijk[i];
  //	}
...
__global__ void run4(int scaler, int *dpedgesPN, int noGenes, int noEdges,
  ...
  if (index < uniSum * noGenes) {
    out[index] = bDeu3(0, 0, 0, 0, stateSpace, stateFlag, dpedgesPN, index,
                       noGenes, noEdges, uniSum, numClass1, 4, data1, pedges,
                       pNodes, nij, nijk, index * 27, index * 81, ppnLen);
  }

  else {
    out[index] =
        bDeu3(1, uniSum, (uniSum)*noGenes, noGenes, stateSpace, stateFlag,
              dpedgesPN, index, noGenes, noEdges, uniSum, numClass2, 4, data2,
              pedges, pNodes, nij, nijk, index * 27, index * 81, ppnLen);
  }
}
